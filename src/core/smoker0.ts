export class Smoker<VALUE> implements Disposable {
  ...
  private _deferredAborts: number[] = []; // Stores indices to swap after emission finishes
  ...
  private _flushDeferred() {
    ...
    // Process deferred pop-and-swaps backwards to prevent index shifting bugs
  ...
  listen(listener: Smoker.Listener<VALUE>): Smoker.Abort {
    ...
    return () => {
      ...
      if (this._emitting) {
        // If an emission is reading the array right now, lock it.
        // Save the index to execute an O(1) swap the instant the loop finishes.
        this._deferredAborts.push(idx);
        sub.index = -1;
        return;
      }

      // Standard path: Ultra-fast O(1) Pop-and-Swap
      const last = this._listeners.pop()!;
      if (idx < this._listeners.length) {
        (this._listeners[idx] = last).index = idx;
      }
